...
export default class App extends React.Component {
  ...
  _retrieveData(){
    database.transaction(transaction => transaction.executeSql('SELECT * FROM quotes',[],(_, result) => this.setState({quotes: result.rows._array}) ));
  }

  _saveQuoteToDB(content, author, quotes){
    database.transaction(transaction => transaction.executeSql('INSERT INTO quotes (content, author) VALUES (?,?)', [content, author], (_, result) => (quotes[quotes.length - 1].id = result.insertId) ));
  }

  _removeQuoteFromDB(id){
    database.transaction(transaction => transaction.executeSql('DELETE FROM quotes WHERE id = ?', [id]));
  }
  ...
  _addQuote = (content, author) => {
    ...
    if(content && author){
      ...
      this._saveQuoteToDB(content, author, quotes)
    }
  ...
  _deleteButton(){
   ...
   [
    ...
    { text: 'Si', style: 'destructive', onPress: () => this._deleteQuote() }
   ]);
  ...
  componentDidMount(){
    ...
    database.transaction(transaction => transaction.executeSql('CREATE TABLE IF NOT EXISTS quotes (id INTEGER PRIMARY KEY NOT NULL, content TEXT, author TEXT);'));
